#' ---
#' title: "Area level Example with informative a priori for beta"
...
sigma_v <- 1
sigma_e <- rep(1, N1)
theta <- beta0 + beta1 * x + rnorm(N1, 0, sigma_v)

X <- as.matrix(cbind(1, x))
y <- theta + rnorm(N1, 0, sigma_e)

N2 <- 10
Xs <- as.matrix(cbind(1, rnorm(N2)))

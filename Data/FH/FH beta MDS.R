...
n_eff <- data_dir$n_effec
phi <- data_dir$n_effec - 1
phi[13] <- 2 #Phi[13] es cero y no puede ser as?
...
#+ results='hide'
fit <- fit_FH_Beta$sample(iter_warmup = 250,
                          iter_sampling = 250,
                          chains = 4,
                          parallel_chains = 4,
                          seed = 1234,
...
# library(glmmTMB)
# 
# glmmTMB(pobreza ~ log_rem_mintrab + prom_pob + porcentaje_depoblacinrural +
#           porcentajedeasistenciaescolarcom + analfabeto + pad2009 +
#           pnna2009 + region1 + region2 + region3 + region4 + region5 +
#           region6 + region7 + region8 + region9 + region10 + region11 +
#           region12 + region13 + region14 + region15 - 1 + (1|com_id), 
#           data= data, family = list(family = "beta", link = "logit"), weights =)
...
fitsummary <- fit$summary("thetapred")
...
with(EstimacionesBayes, plot(FH, Bayes))
abline(a = 0, b = 1, col = 2)

with(estimaciones, plot(FH, Bayes))
abline(a = 0, b = 1, col = 2)

with(predicciones, plot(FH, Bayes))
abline(a = 0, b = 1, col = 2)
